...
def delete_network(conn, network_name):
    ...
    if network:
        ...
        for subnet in subnets:
            # Delete all ports associated with the subnet
            ports = list(conn.network.ports())
            for port in ports:
...
def delete_router(conn, router_name):
    ...
    if router:
        ...
        # Detach subnets/interfaces from the router
        detach_subnets_from_router(conn, router_id)

        # Delete internal ports associated with the router
        delete_ports_associated_with_router(conn, router_id)

        # Remove external gateway (if any)
...
def delete_floating_ips(conn):
    ...
    for ip in floating_ips:
        if ip.fixed_ip_address is None:  # Only delete unassigned floating IPs
            try:
...
def main(openrc, tag, ssh_key):
    ...
    # Delete network and associated resources
    delete_network(conn, tag + "_network")

    # Clean up routers (if any)
